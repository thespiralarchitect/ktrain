...
func main() {
	...
	if err != nil {
		logger.InitLogger().Fatalf("Failed to connect to RabbitMQ, err: %v", err)
		return
	}
	...
	r.Get("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			logger.InitLogger().Errorw("Error to write response,err:%v", err)
	...
	if err != nil {
		logger.InitLogger().Fatalf("ListenAndServe(): %v", err)
		return
	}
}
